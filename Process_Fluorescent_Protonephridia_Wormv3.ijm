...
selectWindow("slc");
count_high_slc=count_high/(4000/slc_scaler);
count_low_slc=count_low/(4000/slc_scaler);
area_high_slc=area_high/(4000/slc_scaler);
area_low_slc=area_low/(4000/slc_scaler);
run("Subtract Background...", "rolling=50");
run("Smooth");
run("Enhance Local Contrast (CLAHE)", "blocksize=49 histogram=256 maximum=4 mask=Mask");

//Create counting image
run("Hysteresis...", "high="+count_high_slc+" low="+count_low_slc);
rename("PrePeaks");
...
//Create area image
selectWindow("slc");
run("Hysteresis...", "high="+area_high_slc+" low="+area_low_slc);
rename("PreArea");
run("Analyze Particles...", "size="+minimum_proto_size+"-Infinity circularity=0.00-1.00 show=Masks display clear add");
...
selectWindow("CAVII");
count_high_cavII=count_high/(4000/cavII_scaler);
count_low_cavII=count_low/(4000/cavII_scaler);
area_high_cavII=area_high/(4000/cavII_scaler);
area_low_cavII=area_low/(4000/cavII_scaler);
run("Subtract Background...", "rolling=50");
run("Smooth");
run("Enhance Local Contrast (CLAHE)", "blocksize=49 histogram=256 maximum=4 mask=Mask");

//Create counting image
run("Hysteresis...", "high="+count_high_cavII+" low="+count_low_cavII);
rename("PrePeaks");
run("Analyze Particles...", "size="+minimum_proto_size+"-Infinity circularity=0.00-1.00 show=Masks display clear add");
...
//Create area image
selectWindow("CAVII");
run("Hysteresis...", "high="+area_high_cavII+" low="+area_low_cavII);
rename("PreArea");
run("Analyze Particles...", "size="+minimum_proto_size+"-Infinity circularity=0.00-1.00 show=Masks display clear add");
