...
else
{
	...
	run("Split Channels");
	selectWindow("C1-"+t);
	run("Scale Ramp Zstack", "background=100 final=20 exponential?");
...
setThreshold(1200, 640000000.0000);
run("Convert to Mask", "method=Default background=Dark black");
rename("Mask");
...
run("Compute 3D Blob Statistics Round", "mask=Mask first=20 lateral=10 threshold=10000000000");
