...
for (n=0; n<source_list.length; n++)
{
	...
	if (File.isDirectory(cur_file)==1)
	{
		...
		for (m=0; m<list.length; m++) 
		{
			...
			if (endsWith(list[m],"mvd2")==1&&endsWith(list[m], "processed.tif")==0&&endsWith(list[m],".tif")==0)
			{
				...
				logs=runMacro("U:\\smc\\Fiji_2016.app\\macros\\BBP_StitchLargeWormUltraview.ijm", cur_file+list[m]);
