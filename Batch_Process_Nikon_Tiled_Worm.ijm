...
print(f, "Folder"+","+ "File"+","+"Spots"+","+ "Area");
for (n=0; n<source_list.length; n++)
{
	...
	if (File.isDirectory(cur_file)==1)
	{
		...
		for (m=0; m<list.length; m++) 
		{
			...
			if (endsWith(list[m],"_projection.tif")==1&&endsWith(list[m], "processed.tif")==0&&endsWith(list[m],".tif")==1)
			{
				...
				logs=runMacro("/n/projects/smc/Fiji_2016.app/macros/ProcessSingleWormFindMaximaMultipleChannelsCPAv2.ijm", cur_file+list[m]);
				//logs=runMacro("U:\\smc\\Fiji_2016.app\\macros\\ProcessSingleWormFindMaximav2.ijm", cur_file+list[m]);
