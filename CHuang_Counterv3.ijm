...
run("32-bit");
rename("BlobFinder");
run("Percentile Threshold", "percentile=30 snr=22");
//return("");
//run("Analyze Particles...", "size=0-8000 circularity=0.40-1.00 show=Masks");
run("Extended Particle Analyzer", "pixel roundness=0.2-1.0 show=Masks redirect=None keep=None");
run("Invert LUT");
run("Watershed");
