...
tmp_dir = getDirectory("Choose a Directory ");	//get directory

for(tttt = 0; tttt < list.length; tttt++) 
{		//loop through each element in the 'list' array
	...
	else
	{
		...
		run("32-bit");
		run("Stack to Hyperstack...", "order=xyczt(default) channels="+cc+" slices="+ss+" frames="+ff+" display=Grayscale");
