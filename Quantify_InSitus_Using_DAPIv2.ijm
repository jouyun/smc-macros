...
run("Split Channels");
selectWindow(ttt);
run("Duplicate...", "title=DAPI duplicate channels=4 slices=4");
run("Merge Channels...", "c1=C1-SUM_"+ttt+" c2=C2-SUM_"+ttt+" c3=C3-SUM_"+ttt+" c4=DAPI create");

saved_file = substring(current_file, 0, lengthOf(current_file)-4)+".tif";
run("Save As Tiff", "save="+saved_file);

run("Cellpose Infer", "source="+saved_file+" diameter=40 normalize=-1 normalize_0=-1 type=cyto channel=3");
tt=getTitle();
