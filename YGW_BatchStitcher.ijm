...
{
	dir=source_dir+source_list[n];
	if (File.isDirectory(dir))
	{
		s=0;
		for (i=0; i<4; i++)
		{
			run("Bio-Formats Importer", "open="+dir+"NDExp_Point"+IJ.pad(i,4)+"_Count00000_Seq"+IJ.pad(i*2,4)+".nd2 autoscale color_mode=Default rois_import=[ROI manager] view=Hyperstack stack_order=XYCZT");
			run("Bio-Formats Importer", "open="+dir+"NDExp_Point"+IJ.pad(i,4)+"_Count00001_Seq"+IJ.pad(i*2+1,4)+".nd2 autoscale color_mode=Default rois_import=[ROI manager] view=Hyperstack stack_order=XYCZT");
			Stack.getDimensions(w,h,c,s,f);
			run("Concatenate...", "all_open title=[Concatenated Stacks]");
			run("Stack to Hyperstack...", "order=xyczt(default) channels=3 slices="+(2*s)+" frames=1 display=Color");
