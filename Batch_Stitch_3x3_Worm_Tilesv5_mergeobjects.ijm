...
for (m=0; m<list.length; m++)
{
    ...
    run("Paste Projection To Front", "channel=2");
    ...
    for (j=1; j<=number_worms; j++)
    {
    	...
    	run("Duplicate...", "title=Subset duplicate frames="+((j-1)*x_dim*y_dim+1)+"-"+(j*x_dim*y_dim));
      	runMacro("SaveMultipageImageSequence.ijm", tmp_directory);
       	tmpT=getTitle();

		//run("Duplicate...", "duplicate channels=1");
        run("merge objects in tiles", "snr=40 hit=10000 border=100 in=100 x="+x_dim+" y="+y_dim+" path="+tmp_directory);
		ctr=0;
		while (isOpen("Fused_"+(ctr+1))) ctr++;
		most_pix=0;
		biggest_fused=-1;
		for (f=0; f<ctr; f++)
		{
			IJ.log("Fused_"+(f+1)+" testing");
			selectWindow("Fused_"+(f+1));
			test_pix=getWidth()*getHeight();
			if (test_pix>most_pix) 
			{
				IJ.log("New biggest: "+(f+1));
				test_pix=most_pix;
				biggest_fused=f+1;
			}
		}
		for (f=0; f<ctr; f++)
		{
			IJ.log("Fused_"+(f+1)+" closing");
			selectWindow("Fused_"+(f+1));
			if (f+1!=biggest_fused) close();
		}
		IJ.log("Fused_"+(f+1)+" saving");
        selectWindow("Fused_"+biggest_fused);
        IJ.log("made it");
        run("Delete Slice");
        IJ.log("About to try c,z,t:  "+channels+","+slices+","+1);
