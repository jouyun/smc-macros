...
close();
run("Grid/Collection stitching", "type="+arrangement+" grid_size_x="+grid_x+" grid_size_y="+grid_y+" tile_overlap="+overlap+" first_file_index_i=0 directory=["+temp_folder+"] file_names=Img{iiii}.tif output_textfile_name=TileConfiguration.txt fusion_method=["+blend_method+"] regression_threshold=0.30 max/avg_displacement_threshold=2.50 absolute_displacement_threshold=3.50"+compute_overlap+" computation_parameters=[Save computation time (but use more RAM)] image_output=[Fuse and display]");
...
for (k=1; k<=slices; k++)
{
	for (i=1; i<=channels; i++)
	{
		if (!(channel==i&&slice==k))
		{
			...
			run("Image Sequence... ", "format=TIFF name=Img start=0 digits=4 save=["+temp_folder+"Img0000.tif]");
			close();
			run("Grid/Collection stitching", "type=[Positions from file] order=[Defined by TileConfiguration] directory=["+temp_folder+"] layout_file="+reg_file+" fusion_method=["+blend_method+"] regression_threshold=0.30 max/avg_displacement_threshold=2.50 absolute_displacement_threshold=3.50 computation_parameters=[Save computation time (but use more RAM)] image_output=[Fuse and display]");
			rename("C"+i+"Z"+k);
