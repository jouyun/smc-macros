...
else
{
	...
	run("Scale Ramp Zstack", "background=000 final=20 exponential?");
	run("Merge Channels...", "c1=C1-"+t+" c2=C2-"+t+" c3=C3-"+t+" c4=C4-"+t+" create");
	rename(t);
...
setThreshold(2000, 640000000.0000);
run("Convert to Mask", "method=Default background=Dark black");
rename("Mask");
...
run("Compute 3D Blob Statistics Round", "mask=Mask first=14 lateral=6 threshold=100000000000");
