...
for (n=0; n<source_list.length; n++)
{
	...
	if (File.isDirectory(cur_file)==1)
	{
		...
		for (m=0; m<list.length; m++) 
		{
			...
			if (endsWith(list[m],".tif")==true&&endsWith(list[m], "mask.tif")==0&&endsWith(list[m],".tif")==1&&endsWith(list[m], "aligned.tif")==0&&endsWith(list[m], "combined.tif")==0&&endsWith(list[m], "ummary.tif")==0)
			{
				...
				logs=runMacro("U:\\smc\\Fiji_2016.app\\macros\\ProcessSingleWormFindMaximaMultipleChannels_KLEI.ijm", cur_file+list[m]);
